-- JIUZE POS — Migration 13
-- Restore fractional quantities in complete_sale() while preserving
-- subscription access enforcement introduced by migration 11.
--
-- Migration 11 accidentally replaced the numeric quantity implementation
-- from migration 9 with an integer quantity variable. This caused valid
-- variable-unit sales such as 0.5 kg to be interpreted as 1 unit.
--
-- This migration restores numeric(12,3) quantity handling and keeps the
-- authoritative subscription guard.
...
begin
  ...
  -- Authoritative subscription check. Keep this inside the database
  -- function because frontend checks are not a security boundary.
...
revoke all
on function public.complete_sale(jsonb, text, numeric, text, numeric)
from public;
