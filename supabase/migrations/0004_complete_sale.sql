-- JIUZE POS — Phase 3, Migration 4
-- complete_sale(...): the atomic checkout transaction described in
-- brief §7. This is the only path by which sales, sale_items,
-- payments, and stock_movements rows are ever created — see the
-- grant comments in migration 3.
--
-- SCOPE NOTE: this migration implements complete_sale only. void_sale,
-- return_sale, and a manual adjust_stock function are the equivalent
-- foundation for Phases 6/7 and are intentionally not built yet — the
-- brief asks for "the foundation for complete_sale" specifically in
-- Phase 3, not the full set of transactional functions.

-- ---------------------------------------------------------------------
-- next_receipt_number: business-scoped, gap-tolerant receipt numbering.
--
-- Locks the owning business row for the duration of the caller's
-- transaction so two simultaneous checkouts in the same shop can't be
-- assigned the same number; different businesses never block each
-- other since each locks only its own row. The sales_business_receipt
-- _number_key unique index (migration 1) is the final backstop.
-- ---------------------------------------------------------------------
...
begin
  ...
  return 'R-' || to_char(now(), 'YYYYMMDD') || '-' || lpad((v_count + 1)::text, 4, '0');
end;
$$;
...
-- complete_sale
--
-- p_items: jsonb array of {"product_id": uuid, "quantity": int, "discount": numeric?}
-- p_payment_method: 'cash' | 'mpesa' | 'card'
-- p_payment_amount: amount tendered (for cash, must cover the total)
-- p_payment_reference: optional (used for mpesa)
-- p_discount: sale-level discount (brief §16, §17)
--
-- Returns the receipt as jsonb. Any failure raises an exception, which
-- rolls back everything done so far in this function — there is no
-- path that leaves a partially-completed sale (brief §7).
-- ---------------------------------------------------------------------
...
begin
  ...
  -- Pass 1: validate every line and lock the product rows so
  -- concurrent checkouts can't oversell the same stock.
  ...
  if p_payment_method = 'cash' and p_payment_amount < v_total then
  ...
  values (v_business_id, v_sale_id, p_payment_method, p_payment_amount, p_payment_reference);
