-- JIUZE POS — Migration 9
-- Add fixed-unit and variable-quantity product support.
-- Existing products remain whole-unit products by default (piece).
...
alter table public.products
  ...
  check (unit_type in ('piece', 'pack', 'kg', 'g', 'litre', 'ml'));

comment on column public.products.unit_type is
  'Inventory/selling unit. piece and pack require whole-number quantities; kg, g, litre and ml allow fractional quantities.';
...
-- ---------------------------------------------------------------------
-- complete_sale
-- Same public signature as before; quantity values now support up to
-- three decimal places and are validated against the product unit.
-- ---------------------------------------------------------------------
...
begin
  ...
  loop
    ...
    if v_quantity <> round(v_quantity, 3) then
      raise exception 'Item quantity can have at most 3 decimal places';
    end if;
    ...
    if v_product.unit_type in ('piece', 'pack') and v_quantity <> trunc(v_quantity) then
      raise exception 'Product "%" is sold by % and requires a whole-number quantity', v_product.name, v_product.unit_type;
    end if;
...
-- ---------------------------------------------------------------------
-- adjust_stock
-- Replace the integer-quantity overload with numeric quantities.
-- ---------------------------------------------------------------------
revoke all on function public.adjust_stock(uuid, integer, text, text) from public;
drop function public.adjust_stock(uuid, integer, text, text);
...
begin
  ...
  if p_quantity_delta <> round(p_quantity_delta, 3) then
    raise exception 'Stock adjustment quantity can have at most 3 decimal places';
  end if;
  ...
  if v_product.unit_type in ('piece', 'pack') and p_quantity_delta <> trunc(p_quantity_delta) then
    raise exception 'Product "%" is sold by % and requires a whole-number stock adjustment', v_product.name, v_product.unit_type;
  end if;
