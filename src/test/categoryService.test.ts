...
describe('categoryService', () => {
  ...
  it('trims and creates a category', async () => {
    await expect(createCategory('business-1', '  Beverages  ')).resolves.toMatchObject({
      id: 'cat-1',
      name: 'Beverages',
    });
  ...
  it('rejects category names longer than 100 characters', async () => {
    await expect(createCategory('business-1', 'a'.repeat(101))).rejects.toThrow(
  ...
  it('updates a category with a trimmed name', async () => {
    await expect(updateCategory('cat-1', '  Grocery  ')).resolves.toMatchObject({
      id: 'cat-1',
      name: 'Beverages',
    });
