...
describe('salesHistoryService', () => {
  ...
  it('lists sales and maps database fields', async () => {
    ...
    await expect(listSales('business-1')).resolves.toEqual([{
      id: 'sale-1',
      receiptNumber: 'R-001',
      cashierId: 'user-1',
  ...
  it('gets sale details, items and payment', async () => {
    ...
    const paymentBuilder = makeBuilder({
      data: {
        method: 'cash',
        amount: '200.00',
        reference: null,
    ...
    vi.mocked(supabase.from).mockImplementation(
      (table) =>
        ({
          sales: saleBuilder,
          sale_items: itemsBuilder,
          payments: paymentBuilder,
        }[table] as never),
    )

    await expect(
      getSale('business-1', 'sale-1'),
  ...
  it('rejects when the sale does not belong to the business or does not exist', async () => {
    ...
    vi.mocked(supabase.from).mockImplementation(
      (table) =>
        ({
          sales: saleBuilder,
          sale_items: emptyBuilder,
          payments: paymentBuilder,
        }[table] as never),
    )

    await expect(
      getSale('business-1', 'missing'),
