...
const validateNumbers = (cost: number, selling: number, stock: number, minimum: number) => {
  if (![cost, selling, stock, minimum].every(Number.isFinite)) throw new Error('Product numeric values must be valid numbers.')
  if ([cost, selling, stock, minimum].some((value) => value < 0)) throw new Error('Product numeric values cannot be negative.')
...
export const createProduct = async (input: CreateProductInput): Promise<Product> => {
  ...
  validateNumbers(input.costPrice, input.sellingPrice, input.stockQuantity, input.minimumStock)
...
export const updateProduct = async (id: string, input: UpdateProductInput): Promise<Product> => {
  ...
  const { data: current, error: currentError } = await supabase.from('products').select('cost_price, selling_price, stock_quantity, minimum_stock').eq('id', id).single()
  if (currentError) throw currentError
  validateNumbers(input.costPrice ?? Number(current.cost_price), input.sellingPrice ?? Number(current.selling_price), input.stockQuantity ?? current.stock_quantity, input.minimumStock ?? current.minimum_stock)
