...
export default function Checkout() {
  ...
  if (completed) return <div className="min-h-screen bg-paper px-6 py-8"><div className="max-w-xl mx-auto"><div className="rounded-lg border border-line bg-paper-raised p-8 text-center"><p className="text-xs font-mono uppercase tracking-wide text-market-600">Sale completed</p><h1 className="font-display font-semibold text-3xl text-ink mt-2">{completed.receiptNumber}</h1><p className="text-sm text-ink-muted mt-2">{money(completed.total)} · {completed.paymentMethod.toUpperCase()}</p><div className="mt-6 border-t border-line pt-5 text-sm text-ink"><div className="flex justify-between"><span>Total</span><strong>{money(completed.total)}</strong></div><div className="flex justify-between mt-2"><span>Paid</span><span>{money(completed.amountPaid)}</span></div><div className="flex justify-between mt-2"><span>Change</span><strong>{money(completed.change)}</strong></div></div><div className="flex justify-center gap-3 mt-7"><button type="button" onClick={() => setCompleted(null)} className="rounded-md bg-ink px-4 py-2 text-sm font-medium text-paper">New sale</button><Link to="/products" className="rounded-md border border-line px-4 py-2 text-sm font-medium text-ink-muted">Products</Link></div></div></div></div>
