...
export default function Analytics() {
  ...
  const trendLabels = useMemo(() => data.salesTrend.filter((_, index) => index % 5 === 0 || index === data.salesTrend.length - 1), [data.salesTrend])
  ...
  return <main className="min-h-screen bg-paper px-5 py-6 sm:px-6 lg:px-8">
    <div className="max-w-[1500px] mx-auto">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4 mb-5">
        <Metric label="Today" value={money(data.todayRevenue)} detail={`${money(data.todayProfit)} gross profit`} />
        <Metric label="This week" value={money(data.weekRevenue)} detail={`${money(data.weekProfit)} gross profit`} />
        <Metric label="This month" value={money(data.monthRevenue)} detail={`${data.monthTransactions} transactions`} />
        <Metric label="Gross margin" value={`${data.grossMargin.toFixed(1)}%`} detail={`Avg sale ${money(data.averageSale)}`} />
      </section>

      <section className="grid gap-5 xl:grid-cols-[1.6fr_0.9fr] mb-5">
        <div className="rounded-2xl border border-line bg-paper-raised p-5 sm:p-6">
          <div className="flex items-start justify-between gap-4 mb-6"><div><h2 className="font-display font-semibold text-lg text-ink">Sales performance</h2><p className="text-xs text-ink-muted mt-1">Daily revenue for the last 30 days</p></div><div className="text-right"><p className="text-xs text-ink-muted">Current month</p><p className="font-display font-semibold text-lg text-ink mt-1">{money(data.monthRevenue)}</p></div></div>
          <div className="h-56 flex items-end gap-1.5 sm:gap-2 border-b border-line pb-1">
            {data.salesTrend.map((day) => <div key={day.date} className="group flex-1 h-full flex items-end relative"><div title={`${day.date}: ${money(day.revenue)}`} className="w-full min-h-1 rounded-t-md bg-market-600/80 group-hover:bg-market-700 transition-colors" style={{ height: `${Math.max(2, (day.revenue / maxRevenue) * 100)}%` }} /></div>)}
          </div>
          <div className="flex justify-between pt-2 text-[10px] font-mono text-ink-muted">{trendLabels.map((day) => <span key={day.date}>{day.date.slice(5)}</span>)}</div>
        </div>

        <div className="rounded-2xl border border-line bg-paper-raised p-5 sm:p-6">
          <div className="flex items-start justify-between"><div><h2 className="font-display font-semibold text-lg text-ink">Monthly projection</h2><p className="text-xs text-ink-muted mt-1">Based on current daily pace</p></div><span className="text-xs rounded-full bg-market-50 text-market-700 px-2.5 py-1">Estimated</span></div>
          <div className="mt-7"><p className="text-xs uppercase tracking-wide text-ink-muted">Projected revenue</p><p className="font-display font-semibold text-3xl text-ink mt-2">{money(data.projectedMonthRevenue)}</p><div className="h-2 rounded-full bg-paper mt-4 overflow-hidden"><div className="h-full rounded-full bg-market-600" style={{ width: `${projectionProgress}%` }} /></div><div className="flex justify-between text-xs mt-2"><span className="text-ink-muted">{money(data.monthRevenue)} achieved</span><span className="text-ink-muted">{projectionProgress.toFixed(0)}%</span></div></div>
          <div className="border-t border-line mt-6 pt-5"><p className="text-xs uppercase tracking-wide text-ink-muted">Projected gross profit</p><p className="font-display font-semibold text-xl text-ink mt-2">{money(data.projectedMonthProfit)}</p></div>
        </div>
      </section>

      <section className="grid gap-5 lg:grid-cols-3 mb-5">
        <InsightCard label="Inventory value" value={money(data.inventoryValue)} detail="At current cost price" />
        <InsightCard label="Low stock" value={String(data.lowStockCount)} detail={data.lowStockCount ? 'Products need attention' : 'Inventory looks healthy'} alert={data.lowStockCount > 0} />
        <InsightCard label="Average transaction" value={money(data.averageSale)} detail="Current month average" />
      </section>

      <section className="grid gap-5 lg:grid-cols-2">
        <ProductTable title="Top products" subtitle="Highest revenue this month" products={data.topProducts} max={bestProductRevenue} />
        <ProductTable title="Slow-moving products" subtitle="Lowest unit sales this month" products={data.slowProducts} max={Math.max(...data.slowProducts.map((product) => product.units), 1)} units />
      </section>
    </div>
  </main>
}
