...
function buildInsights(data: AnalyticsData): Insight[] {
  ...
  return [
    bestProduct
      ? { label: 'Best performer', title: bestProduct.name, detail: `${money(bestProduct.revenue)} net revenue and ${money(bestProduct.profit)} gross profit this month.`, tone: 'positive' as const }
    ...
    data.monthReturns > 0
      ? { label: 'Returns', title: `${money(data.monthReturns)} refunded`, detail: 'Net sales and profit already exclude these returned transactions.', tone: 'warning' as const }
      : { label: 'Returns', title: 'No returns this month', detail: 'No customer refunds have been recorded in the current month.', tone: 'positive' as const },
    ...
    averageActiveDay > 0
      ? { label: 'Sales pattern', title: `${money(averageActiveDay)} per active day`, detail: 'Use the net sales trend to identify stronger days and repeat what works.', tone: 'neutral' as const }
    ...
    attentionProduct && attentionProduct.id !== bestProduct?.id
      ? { label: 'Slow mover', title: attentionProduct.name, detail: `${quantity(attentionProduct.units)} units sold net this month. Consider reviewing pricing, placement or demand.`, tone: 'warning' as const }
      : null,
