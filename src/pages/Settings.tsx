...
export default function Settings() {
  ...
  return <main className="min-h-screen bg-paper px-5 py-6 sm:px-6 lg:px-8"><div className="max-w-[1100px] mx-auto">
    ...
    {loading ? <div className="rounded-2xl border border-line bg-paper-raised p-14 text-center text-sm text-ink-muted">Loading settings…</div> : <div className="space-y-5">
      <form onSubmit={saveBusiness} className="rounded-2xl border border-line bg-paper-raised overflow-hidden"><div className="p-5 sm:p-6 border-b border-line"><p className="text-xs font-medium uppercase tracking-[0.12em] text-market-600">Store profile</p><h2 className="font-display font-semibold text-xl text-ink mt-2">Business information</h2><p className="text-sm text-ink-muted mt-1">This information appears across your POS account.</p></div><div className="p-5 sm:p-6 grid gap-4 sm:grid-cols-2"><label className="sm:col-span-2"><span className="field-label">Business name</span><input required value={businessForm.name} onChange={(e) => setBusinessForm({ ...businessForm, name: e.target.value })} maxLength={120} className="field" /></label><label><span className="field-label">Phone</span><input value={businessForm.phone} onChange={(e) => setBusinessForm({ ...businessForm, phone: e.target.value })} maxLength={50} className="field" /></label><label><span className="field-label">Location</span><input value={businessForm.location} onChange={(e) => setBusinessForm({ ...businessForm, location: e.target.value })} maxLength={200} className="field" /></label></div><div className="px-5 sm:px-6 py-4 border-t border-line flex justify-end"><button disabled={savingBusiness} className="rounded-lg bg-ink px-5 py-2.5 text-sm font-medium text-paper disabled:opacity-50">{savingBusiness ? 'Saving…' : 'Save business settings'}</button></div></form>
      <form onSubmit={saveBusiness} className="rounded-2xl border border-line bg-paper-raised overflow-hidden"><div className="p-5 sm:p-6 border-b border-line"><p className="text-xs font-medium uppercase tracking-[0.12em] text-market-600">Inventory rules</p><h2 className="font-display font-semibold text-xl text-ink mt-2">Stock controls</h2><p className="text-sm text-ink-muted mt-1">Control how stock warnings and negative inventory behave.</p></div><div className="p-5 sm:p-6 space-y-5"><label className="block max-w-xs"><span className="field-label">Low-stock threshold</span><input type="number" min="0" step="1" value={lowStockThreshold} onChange={(e) => setLowStockThreshold(e.target.value)} className="field" /><span className="text-xs text-ink-muted mt-1.5 block">Products at or below this quantity are flagged as low stock.</span></label><label className="flex items-start gap-3 rounded-xl border border-line bg-paper p-4 cursor-pointer"><input type="checkbox" checked={allowNegativeStock} onChange={(e) => setAllowNegativeStock(e.target.checked)} className="mt-1 h-4 w-4" /><span><span className="block text-sm font-medium text-ink">Allow negative stock</span><span className="block text-xs text-ink-muted mt-1">Permit sales to take inventory below zero. Keep this off for normal retail stock control.</span></span></label></div><div className="px-5 sm:px-6 py-4 border-t border-line flex justify-end"><button disabled={savingBusiness} className="rounded-lg bg-ink px-5 py-2.5 text-sm font-medium text-paper disabled:opacity-50">{savingBusiness ? 'Saving…' : 'Save inventory settings'}</button></div></form>
      <form onSubmit={changePassword} className="rounded-2xl border border-line bg-paper-raised overflow-hidden"><div className="p-5 sm:p-6 border-b border-line"><p className="text-xs font-medium uppercase tracking-[0.12em] text-market-600">Account security</p><h2 className="font-display font-semibold text-xl text-ink mt-2">Change password</h2><p className="text-sm text-ink-muted mt-1">Update the password for {user?.email ?? 'your owner account'}.</p></div><div className="p-5 sm:p-6 grid gap-4 sm:grid-cols-2"><label><span className="field-label">New password</span><input required type="password" minLength={8} autoComplete="new-password" value={passwordForm.password} onChange={(e) => setPasswordForm({ ...passwordForm, password: e.target.value })} placeholder="At least 8 characters" className="field" /></label><label><span className="field-label">Confirm new password</span><input required type="password" minLength={8} autoComplete="new-password" value={passwordForm.confirm} onChange={(e) => setPasswordForm({ ...passwordForm, confirm: e.target.value })} placeholder="Repeat new password" className="field" /></label></div><div className="px-5 sm:px-6 py-4 border-t border-line flex items-center justify-between gap-4"><p className="text-xs text-ink-muted">Use a unique password you do not reuse elsewhere.</p><button disabled={savingPassword} className="shrink-0 rounded-lg bg-ink px-5 py-2.5 text-sm font-medium text-paper disabled:opacity-50">{savingPassword ? 'Updating…' : 'Change password'}</button></div></form>
