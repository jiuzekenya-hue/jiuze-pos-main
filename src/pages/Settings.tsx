...
export default function Settings() {
  ...
  const saveBusiness = async (event: FormEvent) => {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : 'Unable to save business settings.') } finally { setSavingBusiness(false) }
  }
  ...
  const saveInventory = async (event: FormEvent) => {
    ...
    setSavingInventory(true); setError(''); setMessage('')
    try {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : 'Unable to save inventory settings.') } finally { setSavingInventory(false) }
  }

  const changePassword = async (event: FormEvent) => {
    event.preventDefault(); setError(''); setMessage('')
    if (!passwordForm.current) { setError('Enter your current password to continue.'); return }
    if (passwordForm.password.length < 8) { setError('New password must be at least 8 characters.'); return }
    if (passwordForm.password === passwordForm.current) { setError('New password must be different from your current password.'); return }
    if (passwordForm.password !== passwordForm.confirm) { setError('New passwords do not match.'); return }
    setSavingPassword(true)
    try {
      const { error: passwordError } = await supabase.auth.updateUser({ password: passwordForm.password, current_password: passwordForm.current })
      if (passwordError) throw new Error(passwordError.message)
      setPasswordForm({ current: '', password: '', confirm: '' })
      setMessage('Password changed successfully. A security notification will be sent to your account email.')
    } catch (err) { setError(err instanceof Error ? err.message : 'Unable to change password. Check your current password and try again.') } finally { setSavingPassword(false) }
  }
